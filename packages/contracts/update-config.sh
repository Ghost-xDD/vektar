...
if [ -f "$CRE_ENV_PATH" ] || [ -L "$CRE_ENV_PATH" ]; then
    ...
    sed "s|^HORIZON_VAULT_ADDRESS=.*|HORIZON_VAULT_ADDRESS=$VAULT_ADDRESS|" "$CRE_ENV_PATH.tmp" > "$CRE_ENV_PATH.tmp2"
    mv "$CRE_ENV_PATH.tmp2" "$CRE_ENV_PATH"
    rm -f "$CRE_ENV_PATH.tmp"
...
if [ -f "$DASHBOARD_ENV_PATH" ] || [ -L "$DASHBOARD_ENV_PATH" ]; then
    ...
    # Use sed without -i for symlinks
    sed "s|^VITE_VAULT_ADDRESS=.*|VITE_VAULT_ADDRESS=$VAULT_ADDRESS|" "$DASHBOARD_ENV_PATH" > "$DASHBOARD_ENV_PATH.tmp"
    sed "s|^VITE_ESCROW_ADDRESS=.*|VITE_ESCROW_ADDRESS=$ESCROW_ADDRESS|" "$DASHBOARD_ENV_PATH.tmp" > "$DASHBOARD_ENV_PATH.tmp2"
    sed "s|^VITE_USER_ADDRESS=.*|VITE_USER_ADDRESS=$DEPLOYER_ADDRESS|" "$DASHBOARD_ENV_PATH.tmp2" > "$DASHBOARD_ENV_PATH.tmp3"
    mv "$DASHBOARD_ENV_PATH.tmp3" "$DASHBOARD_ENV_PATH"
    rm -f "$DASHBOARD_ENV_PATH.tmp" "$DASHBOARD_ENV_PATH.tmp2"
